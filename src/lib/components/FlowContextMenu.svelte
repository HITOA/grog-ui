<script lang="ts">
    ...
    function getTreeChildCountRecursive(node: TreeNode): number {
        let result = 0;
        node.files?.forEach(child => {
            if (child.files)
                result += getTreeChildCountRecursive(child);
            else
                result += 1;
        });
        return result;
...
{#snippet folder(node: TreeNode)}
    <ContextMenu.Sub>
        <ContextMenu.SubTrigger class="menu-item">
            ...
            <span class="menu-item-file-count">{getTreeChildCountRecursive(node)}</span>
...
<ContextMenu.Root bind:open={contextMenuOpenState}>
    ...
    <ContextMenu.Portal>
        <ContextMenu.Content class="menu-frame" style="z-index: 2;" forceMount>
            {#snippet child({ wrapperProps, props, open })}
                {#if open}
                    <div {...wrapperProps}>
                        <div {...props} transition:fly={{ duration: 100, y: -20 }}>
                            <Command.Root>
